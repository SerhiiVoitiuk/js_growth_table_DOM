...
appendRowBtn.addEventListener("click", ()=>{
    if (table.rows.length < maxRows) {
        ...
        for(let i = 0; i < table.rows[0].cells.length; i++)newRow.insertCell().textContent = "";
    }
    updateButtonState();
});
...
appendColumnBtn.addEventListener("click", ()=>{
    if (table.rows[0].cells.length < maxColumns) Array.from(table.rows).forEach((row)=>{
        row.insertCell().textContent = "";
    });
    updateButtonState();
});
removeColumnBtn.addEventListener("click", ()=>{
    if (table.rows[0].cells.length > minColumns) Array.from(table.rows).forEach((row)=>{
        row.deleteCell(-1);
    });
    updateButtonState();
});
